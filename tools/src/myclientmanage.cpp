...
MyClientManage::MyClientManage(char *serverAddress,int perSendSize,int blockSize,
		...
		double zeta,double sigma,double forZeta,double forSigma,int hotPlaces,int playToPlay,int playToPause,int playToForward,
		...
		int period,int lrfuLambda,bool isRepeat){
	...
	cout << "HotPlaces " << hotPlaces << endl;

	for(int i = 0;i < devNums;i++){
		LOG_INFO("Cluster " << i << " address is:" << mClusterAddress[i]);
	}

	mModel = new ModelAssemble(mFileNum,mThelta,mLambda,mZeta,mSigma,forZeta,forSigma,mClientNums,
			mPlayToPlay,mPlayToPause,mPlayToForward,mPlayToBackward,mPlayToStop,mIsStartTogether,hotPlaces);
}
